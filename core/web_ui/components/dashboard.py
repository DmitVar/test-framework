...
class Dashboard(BaseComponent):
    def __init__(self, page: Page, dashboard_data_qa_value: str):
        super().__init__(page)
        ...
        self.dashboard_title = Text(
            page, name="Dashboard Title", locator=f"{self.locator_string} p.stat-label"
        )

        self.dashboard_value = Text(
            page, name="Dashboard Value", locator=f"{self.locator_string} p.stat-label"
        )
