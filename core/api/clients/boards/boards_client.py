...
class BoardsClient(APIClient):
    def get_all_public_boards(self) -> Response:
        return self.get(
            url=f"{settings.http_client.url}boards/public",
        )

    def get_public_board_by_id(self, board_id: int) -> Response:
        return self.get(
            url=f"{settings.http_client.url}boards/public/{board_id}",
        )
    ...
    def get_current_user_boards(
        ...
        return self.get(
            url=f"{settings.http_client.url}boards/",
            params=query_params,
        )
    ...
    def get_board_by_id(self, board_id: int) -> Response:
        return self.get(
            url=f"{settings.http_client.url}boards/{board_id}",
        )

    def update_board_by_id(
        self, request: RequestUpdateBoardSchema, board_id
    ) -> Response:
        return self.put(
            url=f"{settings.http_client.url}boards/{board_id}",
            json=request.model_dump(by_alias=True),
    ...
    def add_member_to_board_by_id(self, board_id: int, member_id: int) -> Response:
        return self.post(
            url=f"{settings.http_client.url}boards/{board_id}/members/{member_id}"
        )
    ...
    def get_board_members(self, board_id: int) -> Response:
        return self.get(
            url=f"{settings.http_client.url}boards/{board_id}/members",
        )

    def move_board_to_the_archive(self, board_id: int) -> Response:
        return self.put(
            url=f"{settings.http_client.url}boards/{board_id}/archive",
        )

    def get_board_stats(self, board_id: int) -> Response:
        return self.get(
            url=f"{settings.http_client.url}boards/{board_id}/stats",
        )
