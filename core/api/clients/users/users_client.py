...
class UsersClient(APIClient):
    def get_public_users(self)->Response:
        return self.get(
            url=f"{settings.http_client.url}users/public"
        )

    def get_current_user(self)->Response:
        return self.get(
            url=f"{settings.http_client.url}users/me"
        )
    ...
    def update_user_api(self, request: RequestUpdateUserSchema, user_id: int)->Response:
        return self.put(
    ...
    def get_tasks_current_user(self, skip: int = 0, limit: int = 100)->Response:
        ...
        return self.get(
            url=f"{settings.http_client.url}users/me/tasks",
            params=query_params
        )

    def update_user_avatar(self, request: RequestUpdateUserAvatarSchema, user_id: int)->Response:
        return self.put(
